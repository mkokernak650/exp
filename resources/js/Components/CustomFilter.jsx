...
const CustomFilter = (props) => {
    const { mainData, fields, filterValue, setFilterValue, filteredData, setFilteredData, filterData } = props
    ...
    const lastFkeyOfArray =
        filterValue?.items[filterValue?.items?.length - 1]?.fkey;
    ...
    const optionsFields = (field) => {
        const arrayUniqueByKey = [...new Map(mainData.map(value =>
    ...
    const [options, setOptions] = useState(optionsFields(filterValue?.items?.[0]?.field));

    console.log('optionsFields', optionsFields('lname'));
        ...
        <>
            <div className="custom-filter">
                <div className="groups">
                    ...
                    <div className="add">
                        <span className="add-icon" onClick={addCondition}>+ Add Condition</span>
                    </div>
                </div>
                ...
                {filterValue.items.map((cItem, indx) => (
                    <div className="item" key={indx}>
                        {/* {console.log(
              '%cRender',
              'background:blue;color: white;padding:5px 10px;border-radius:5px'
            )} */}
                        ...
                        <div className={`value ${cItem.operator !== 'isEmpty' &&
                            cItem.operator !== 'isNotEmpty'
                            ? 'text-field-show'
                            : 'text-field-hide'}`}>
