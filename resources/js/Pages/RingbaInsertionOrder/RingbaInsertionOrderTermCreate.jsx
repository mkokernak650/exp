...
const RingbaInsertionOrderTermCreate = () => {
  ...
  const getDataByCampaign = (campaignId) => {
    ...
    axios
      .post(route('insertion.order.ringba.term.data.by.campaign'), { campaignId })
      .then((response) => {
  ...
  const handleInsertionOrderFor = (e) => {
    ...
    if (e.target.value === 'affiliate') {
      ...
      if (selectedAffiliate) {
        ...
        const phoneOptionByAffiliate = phoneOptions.filter(
          (item) => item.affiliateId === selectedAffiliate.split(',')[0]
        )
          ...
          <Row gutter={[0, 16]}>
            ...
            <Row justify="end" className="w-full">
              <Col className="mr-2">
                <Button
                  disabled={
                    (insertionOrderFor === 'affiliate' &&
                      (!selectedAffiliate || !selectedPayout)) ||
                    (insertionOrderFor === 'customer' && (!selectedCustomer || !selectedRevenue)) ||
                    !selectedCampaign ||
                    !selectedPhone ||
                    loading.submit ||
                    loading.save ||
...
RingbaInsertionOrderTermCreate.layout = (page) => (
  <Layout title="Pay Per Call Insertion Order Term - Create">{page}</Layout>
)
