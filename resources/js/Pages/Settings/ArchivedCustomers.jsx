...
const ArchivedCustomers = () => {
  ...
  const handleActive = () => {
    axios
      .post(route('active.customer'), { selectedRowIds: selectedRowKeys })
      .then((res) => {
  ...
  return (
    <>
        ...
        <div className="edit_target">
          <form>
            <div className="mb-4">
              ...
              />
              {errors?.customer && (
                <div className="text-red-500 text-xs">{errors?.customer?.[0]}</div>
              )}
            </div>
            <div className="mb-4">
              <label>Email</label>
      ...
      <ConfirmModal
        ...
        width={'450px'}
        title={`${selectedRowKeys.length > 1 ? 'Do you want to active these customers?' : 'Do you want to active this customer?'}`}
      />
    </>
  )
}
