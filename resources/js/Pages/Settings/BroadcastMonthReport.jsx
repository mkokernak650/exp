...
const BroadcastMonthReport = () => {
  ...
  const { BroadCastMonths } = usePage().props;
      ...
      <Checkbox
        ...
        onChange={(event) => {
          if (event.currentTarget.checked) {
            ...
            setTableToolbar(true);
            let i = 0;
            while (i < BroadcastMonthReport.length) {
              selectedRowIds.push(BroadcastMonthReport[i].id);
              i++;
            }
          } else {
  ...
  const OPTION_KEY = "broadcast-month-report";
  const stateStore = {
    ...tablePropsInit,
...
BroadcastMonthReport.layout = (page) => (
  <Layout title="BroadcastMonthReport">{page}</Layout>
);
