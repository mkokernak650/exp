...
import { fields, filter, columns as defaultColumns } from './Helpers/BroadcastGroupNamesProps'

const BroadcastGroupNames = () => {
  const { allBroadcastGroupNames, columnsData } = usePage().props
  ...
  const dataArray = (allBroadcastGroupNames.data || []).map((item, index) => ({
    edit: item.id,
    sl: index + 1,
    broadcast_group_name: item.broadcast_group_name,
    status: [item.status, item.id],
    id: item.id,
    key: item.id,
  }))

  const optionKey = 'broadcast-group-names-report'
  ...
  const handleStatus = (value, rowId) => {
    axios
      .post(route('broadcast_group_names.status.update'), { value: value, rowId: rowId })
      .then((res) => {
        setData((prev) =>
          prev.map((item) => {
            if (item.id === rowId) {
  ...
  const getSearchingData = async (page = 1) => {
    setCurrentPage(page)
    await axios
      .get(`/broadcast-group-names-report?page=${page}&itemPerPage=${itemPerPage}`)
      .then((res) => {
        setData(
  ...
  const deleteHandler = () => {
    axios
      .post(route('broadcast_group_names.delete'), { selectedRowIds: selectedRowKeys })
      .then((res) => {
  ...
  const handleEditSubmit = () => {
    axios
      .post(route('broadcast_group_name.edit'), editData)
      .then((res) => {
        if (res.data.status_code === 200) {
          setData((prev) =>
            prev.map((item) => {
              if (item.id === editData.id) {
                return { ...item, broadcast_group_name: editData.broadcast_group_name }
              }
              return item
            })
          )
  ...
  return (
    <>
      <Helmet title="Broadcast Group Names Report" />
      ...
      <NormalModal
        ...
        width={'600px'}
        title={'Edit Broadcast Group Name'}
        onClose={() => handleCloseModal(setShowEditModal)}
      >
        <div className="edit_target">
          <form>
            <span>Broadcast Group Name:</span>
            <Input
              value={editData ? editData.broadcast_group_name : ''}
...
BroadcastGroupNames.layout = (page) => <Layout title="Broadcast Group Names">{page}</Layout>
export default BroadcastGroupNames
