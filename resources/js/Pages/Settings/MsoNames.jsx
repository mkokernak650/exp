...
const MsoNames = () => {
  const { allMsoNames, columnsData } = usePage().props
  ...
  const [totalRecords, setTotalRecords] = useState(allMsoNames.total || 0)

  const dataArray = (allMsoNames.data || []).map((item, index) => ({
    edit: item.id,
    sl: index + 1,
  ...
  const optionKey = 'mso-names-report'
  ...
  const handleStatus = (value, rowId) => {
    axios
      .post(route('mso_names.status.update'), { value: value, rowId: rowId })
      .then((res) => {
        setData((prev) =>
          prev.map((item) => {
            if (item.id === rowId) {
  ...
  const getSearchingData = async (page = 1) => {
    setCurrentPage(page)
    await axios
      .get(`/mso-names-report?page=${page}&itemPerPage=${itemPerPage}`)
      .then((res) => {
        setData(
  ...
  const deleteHandler = () => {
    axios
      .post(route('mso_names.delete'), { selectedRowIds: selectedRowKeys })
      .then((res) => {
  ...
  const handleEditSubmit = () => {
    axios
      .post(route('mso_name.edit'), editData)
      .then((res) => {
        if (res.data.status_code === 200) {
          setData((prev) =>
            prev.map((item) => {
              if (item.id === editData.id) {
                return { ...item, mso_name: editData.mso_name }
              }
              return item
            })
          )
      ...
      >
        <div className="mt-4">
          <form>
            <span>MSO Name:</span>
            <Input
              value={editData ? editData.mso_name : ''}
              name="mso_name"
              type="text"
...
MsoNames.layout = (page) => <Layout title="MSO Names">{page}</Layout>
export default MsoNames
