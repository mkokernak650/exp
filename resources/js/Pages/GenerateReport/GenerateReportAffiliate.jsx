...
const GenerateReportAffiliate = () => {
  ...
  const fileName = `${values?.type}_Report${values.customer_name ? `_For_Customers(${values.customer_name})` : ''
      ...
      ? `_For_(${values.start_date.toString()}_To_${values?.end_date.toString()})`
      : ''
  ...
  const handleSubmit = () => {
    setLoading(true)
    axios
      .post(route('affiliate.report.generator'), values)
      .then((res) => {
        setLoading(false)
        if (res.status === 204) {
