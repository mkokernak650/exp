...
const CustomerRoster = () => {
  const { customer, rows } = usePage().props
  ...
  const columns = [
    ...
    { title: 'Accepted', dataIndex: 'accepted_at', key: 'accepted_at' },
    {
      ...
      render: (v) =>
        v ? (
          <Tooltip title="Cancellation effective date (sales tracked until this date)">
    ...
    { title: 'Returns #', dataIndex: 'return_count', key: 'return_count', align: 'right' },
    {
      ...
      render: (v) =>
        v ? (
          ...
          <Tooltip title="No sales or returns in this IO window — could indicate a station outage">
  ...
  return (
    <>
      <Helmet title={`Roster — ${customer?.customer_name || 'Customer'}`} />
      <div className="p-6">
        <Title level={4} className="!mb-1">
          {customer?.customer_name} — IO Roster
        </Title>
        <div className="text-sm text-gray-500 mb-4">
          Every IO under this customer, with attached affiliates and sales scoped to the IO window.
          Zero-activity rows are shown by default so technical outages stay visible.
        </div>
        ...
        <Space className="mb-3 flex-wrap" size="middle">
          <Input
            ...
            placeholder="Search IO / affiliate / market"
            value={search}
...
CustomerRoster.layout = (page) => <Layout title="Customer Roster">{page}</Layout>
