...
class ReportGeneratorController extends Controller
{
    ...
    public function destinationReport(Request $request)
    {
        ...
        if ($request->report_type === 'email-report') {
            ...
            $sendMailCtrl->SendMail($destinationReport, $newSummary, [], $request->file_name, $request->emails, $emailCriteria, $header);
            return;
        }
    ...
    public function callLengthReport(Request $request)
    {
        ...
        $header = $this->getHeader('Call Length', $campaign, $dateRangeForHeader);
        ...
        if (isset($request->year) && $request->start_date === null) {
            $call_summary['Date Range'] = 'Years(' . implode(',', $request->year) . ')';
        }
        ...
        if ($request->report_type === 'email-report') {
            ...
            $separatedFileName = explode('_CallLength_Report', $request->fileNameForEmailCriteria);
    ...
    public function affiliateReport(Request $request)
    {
        ...
        if (!empty($archived)) {
            foreach ($archived as $archive) {
                ...
                if (empty($annotationTag)) {
                    $archive_call['qty'] += 1;
                    $archive_call['revenue'] += (int) $archive->$payout_amount;
                }
                if (!empty($annotationTag)) {
                    array_push($annotation_tags_array, $annotationTag);
                }
        ...
        if (!empty($exceptions)) {
            foreach ($exceptions as $exception) {
                ...
                if (empty($annotationTag)) {
                    $archive_call['qty'] += 1;
                    $archive_call['revenue'] += (int) $exception->$payout_amount;
                }
                if (!empty($annotationTag)) {
                    array_push($annotation_tags_array, $annotationTag);
                }
        ...
        if ($request->report_type === 'email-report') {
            ...
            $sendMailCtrl = new SendMailController();
            $sendMailCtrl->SendMail(collect($newData), $newSummary, $tag_count, $request->file_name, $request->emails, $emailCriteria, $header);
            return;
        }
        return [
            'data'         => $newData,
